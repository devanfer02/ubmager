...
@section('content')
  <section class="container card tw-border-2 tw-border-cst-black tw-bg-cst-yellow ">
    ...
    <form class="tw-p-5" action="{{ route('orders.update', $order) }}" method="post">
      @csrf
      @method('PUT')
      <x-input
        type="text"
      ...
      <div class="mb-3">
        ...
        <textarea
          ...
          cols="30"
          rows="8"
          class="form-control shadow-none tw-border tw-border-cst-black" style="resize:none;">{{ $order['detail'] }}</textarea>
        @error('detail')
